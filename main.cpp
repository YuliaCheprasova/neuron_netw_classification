...
int main()
{
    ...
    srand(time(0));
    int M=60000, n, i, j, k, m, numiter = 1000, numenter = 784, numoutput = 10, L = 3, i_max=0; //(2 скрытых, 1 входной, 1 выходной, но веса при этом считаются три раза)
    ...
    for(j = 0; j < numneuron[0]; j++)//проблема в нормализации, потому что min_u = max_u, может для изображений надо как-то по-другому проводить нормализацию?
    {
        //min_u = data[0][j+1];
        //max_u = data[0][j+1];
        for(i = 0; i < M; i++)
            ...
            for(k = 0; k < L+1; k++)
            {
                for(i = 0; i < numneuron[k+1]; i++)//5
                {
                    ...
                    for(j = 0; j < numneuron[k]; j++)//1
                    {
                        ...
                        v[k][i][j] = beta*v[k][i][j]-h*wgrad[k][i][j];//beta поменяй
                        //cout << v[k][i][j] << endl;
                        w[k][i][j] += v[k][i][j];
                    }
                }
